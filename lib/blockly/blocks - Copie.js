...
Blockly.Blocks['default_rule'] = {
  init: function() {
    default_rule = this;
    var if_on = "if";
    var actions_true = new Blockly.FieldDropdown([["if-true", "if-true"], ["on-true", "on-true"]], function(val) {
      if (val == "on-true" ) actions_false.setValue("on-false");
      else actions_false.setValue("if-false");
      return val;
      });
    var actions_false = new Blockly.FieldDropdown([["if-false", "if-false"], ["on-false", "on-false"]], function(val) {
...
Blockly.Blocks['action_cycle_on_off'] = {
  init: function() {
    ...
    this.setColour(160);
    this.appendDummyInput()
        .appendField("Cycle-on-off on/off nb time until count")
        .appendField("(delay")
        .appendField(new Blockly.FieldTextInput(""), "delay")
        .appendField(")");
    this.appendDummyInput()
