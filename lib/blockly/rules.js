...
var list_object_date = [['-:| '+tr('Choose an object date')+' |:-', '']];
var list_object_time = [['-:| '+tr('Choose an object time')+' |:-', '']];
...
var json_data = {};
var workspace = null;
media = './media/'; // ou 'lib/blockly/media/';
...
function init() {
  ...
  list_object_date = [['-:| '+tr('Choose an object date')+' |:-', '']];
  list_object_time = [['-:| '+tr('Choose an object time')+' |:-', '']];
  ...
  if (json_data.read.config.services.ioports && json_data.read.config.services.ioports.ioport) {
    ...
    if( Object.prototype.toString.call( list_ioports_json ) === '[object Array]' ) {
      ...
      if (list_ioports_json && list_ioports_json.id) {
...
function retLinknxStatus( data , xml) {
  ...
  script.remove();
  script = null;
  if (data.read.status.timers) list_timers_json = data.read.status.timers.task;
  else list_timers_json = [];
  ...
  if (data.read.status.rules) list_ruleactive_json = data.read.status.rules.rule;
  else list_ruleactive_json = [];
...
function reqLinknxValidRule () {
  var xml = Blockly.JavaScript.workspaceToCode(workspace);
  /*if (script) script.remove();
  script = null;
  script = document.createElement('script');
  script.type = 'application/javascript';
