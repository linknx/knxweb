...
Blockly.JavaScript['timer_type'] = function(block) {
  ...
  var code = ' type="' + dropdown_timer_type + '"';
  if ( text_offset != '') code = code + ' offset="' + text_offset + '"';
  if ( text_day != '') code = code +  ' day="' + text_day + '"';
  if ( text_month != '') code = code + ' month="' + text_month + '"';
  if ( text_year != '') code = code + ' year="' + text_year + '"';
...
*/
  ...
  // si "value" est de type "object" alors c'est une condition de type : object-compare
    var code = '<condition type="object-compare" id="' + value_id + '" op="' + dropdown_op + '" id2="' + value_id2 + '" trigger="' + checkbox_trigger + '" ></condition>';
  } else {
          ...
          if (option == 'at') {
            ...
            if (!at_exceptionExists) {
              block.appendDummyInput("at_exception")
                   ...
                   .appendField(new Blockly.FieldDropdown([[tr("Not Use"), ""], [tr("TRUE"), "yes"], [tr("FALSE"), "no"]]), "at_exception");
...
Blockly.JavaScript['timer_hour'] = function(block) {
  ...
  var code = ' hour="' + text_hour + '"';
  if ( text_minute != '') code = code + ' min="' + text_minute + '"';
  if ( text_day != '') code = code +  ' day="' + text_day + '"';
  if ( text_month != '') code = code + ' month="' + text_month + '"';
  if ( text_year != '') code = code + ' year="' + text_year + '"';
  return [code, Blockly.JavaScript.ORDER_ATOMIC];
...
Blockly.JavaScript['action_send-sms'] = function(block) {
  ...
  var code = '<action type="send-sms" id="' + value_id + '" value="' + value_value + '" var="true" delay="' + value_delay + '" ></action>\n';
  return code;
};
//Send Email  <action type="send-email"  to="help@example.com" subject="Foreign contaminant ${ext_temp}" var="true" >Intrusion in your shrubbery! ${ext_temp} </action>
...
Blockly.JavaScript['action_send-email'] = function(block) {
  ...
  var code = '<action type="send-email" to="' + value_to + '" subject="' + value_subject + '" var="true" delay="' + value_delay + '" >' + value_message + '</action>\n';
  return code;
};
//Shell Cmd   <action type="shell-cmd" cmd="killall -v eibd" />
