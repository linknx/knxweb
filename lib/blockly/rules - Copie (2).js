...
function loadRule(rule_id) {
  ...
  for (var i=0;i<tab_rules_json.length;i++)
  {
    if (tab_rules_json[i].id == rule_id) {
      ...
      if (tab_rules_json[i].json.active) document.getElementsByName("auto_manu")[0].checked = tab_rules_json[i].json.active == "yes";
      else document.getElementsByName("auto_manu")[0].checked = true;
...
/*
  ...
  if (tab_rules_json[0].json.init)  document.getElementsByName("init-rule")[0].value = tab_rules_json[0].json.init;
  else document.getElementsByName("init-rule")[0].value = "false";
*/
  /*
  if (tab_rules_json[0].json.active) document.getElementsByName("auto_manu")[0].checked = tab_rules_json[0].json.active == "yes";
  else document.getElementsByName("auto_manu")[0].checked = true;
  */
...
function executeActionlist(type) {
  ...
  console.log( 'statements_actionslist_true', statements_actionslist_true);
  if (type) {
    if (statements_actionslist_true) console.log( 'statements_actionslist_true', statements_actionslist_true); //reqLinknx('', '<execute>'+statements_actionslist_true+'</execute>');
  } else {
    if (statements_actionslist_false) console.log( 'statements_actionslist_false', statements_actionslist_false); //reqLinknx('', '<execute>'+statements_actionslist_false+'</execute>');
  }
  return true;
};
function activeRule(_this) {
  // TODO Auto/Manu action : '<write><config><rules><rule id="'+ruleid+'" active="'+active+'" /></rules></config></write>'
  /*
